...
public class ScriptComponent : MonoBehaviour {
	...
	void OnCollisionExit(Collision collisionInfo) {
		Call ("OnCollisionExit", collisionInfo);
	}
    ...
    void OnPlayerDisconnected(NetworkPlayer player){
        Call ("OnPlayerDisconnected",player);
    }

    void OnDisconnectedFromServer(NetworkDisconnection info){
        Call ("OnDisconnectedFromServer",info);
    }
#endif
    // jg add end
	void OnMouseDown() {
    ...
    protected void Call(string func, params object[] args) {
        if (Table != null && Table.HasValue(func)) {
            try {
                // larrow 2017.06.17 ((ScriptFunction)Table.GetValue(func)).call(args);
                var valObj = Table.GetValue(func);
                var funcObj = (Scorpio.Function.ScriptScriptFunction)(valObj);
                //if (!funcObj.IsStaticFunction)
                funcObj.SetTable(Table);
                funcObj.call(args);
            } catch (Exception e) {
